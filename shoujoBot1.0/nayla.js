...
nyoba cuk
`
nayla.sendMessage(thisListMenu)
break
...
if (MyData("status", sender) == true) { var sts = "Premium User"
 var x = "" }
joni = await getBuffer(`${imgSource}`)
captionBuff = `${textLite}`
try {
reply(joni, {caption: captionBuff}) } catch { nayla.sendMessage(joni,{caption: captionBuff})}
}
break
...
nayla.sendMessage(teks, members_id, true)
break
case 'hidetag':        
if (!isGroup) return reply(`GRUP ONLY`)
if (!isGroupAdmins) return reply(`Hanya dapat dilakukan oleh admin grup!`)
...
nayla.sendMessage(from, options, text, true)					 
break
